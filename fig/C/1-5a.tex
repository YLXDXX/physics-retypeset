...
\tikzset{
   ...
   EFielLineArrow/.style args = {#1}{EcolFL,decoration={markings,
          mark=at position 0.5 with {\arrowreversed[rotate=#1]{latex}}},
          postaction={decorate}}
}
\makeatletter
...
\begin{tikzpicture}
  ...
  \fill[azure5](-1.2,-0.2)rectangle(0,0.2);
  \fill[red5](1.2,-0.2)rectangle(0,0.2);
  \node at (-\a,0)[text=white]{\sffamily S};
  \node at (\a,0) [text=white]{\sffamily N};
  % CHARGES
  % \draw[charge+] (-\a,0) circle (5pt) node[text=white,scale=1.0] { $\bm +$};
  % \draw[charge-] (+\a,0) circle (5pt) node[text=white,scale=1.0] { $\bm -$};

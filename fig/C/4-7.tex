...
\begin{document}
\small
\begin{tikzpicture}[>=latex,z={(10:10mm)},x={(-45:5mm)}]
  \def\wave{
...
\end{scope}
  ...
  \begin{scope}[canvas is zy plane at x=0,fill=azure6]
  \wave
  \draw[-stealth](0,0)--(12.5,0)node[right]{$z$};
  \draw[-stealth](0,0)--(0,1.5)node[above]{$x$};
  % \node at (6,-1.5) [transform shape] {magnetic field};
  \end{scope}
  \begin{scope}[canvas is zx plane at y=0,fill=red6]
  % \draw[help lines] (0,-2) grid (12,2);
  \wave
  \draw[-stealth](0,0)--(0,2.0)node[above]{$y$};
